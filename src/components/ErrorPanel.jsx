...
export function ErrorPanel({ error, onRetry, title = 'Não foi possível carregar' }) {
  ...
  return (
    <div style={{ background: C.bg, borderRadius: 14, padding: '32px 20px', border: `1px solid ${C.redLight}`, textAlign: 'center' }}>
      ...
      {onRetry && (
        <button onClick={onRetry} style={{ background: C.greenDp, color: C.bg, border: 'none', borderRadius: 10, padding: '10px 18px', fontSize: 11, fontFamily: 'monospace', fontWeight: 700, letterSpacing: '2px', cursor: 'pointer' }}>
          TENTAR DE NOVO
        </button>
      )}
    </div>
