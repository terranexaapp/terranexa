...
export async function listarOperacoes(talhaoId) {
  const { data, error } = await supabase
    .from('operacoes')
    .select(`id, categoria, tipo, data_operacao, custo_aplicacao, observacoes, receituario_agronomo, receituario_crea, created_at, insumos:operacao_insumos(id, dose, dose_unidade, quantidade_total, custo_total, insumo:insumos(id, nome, classe, unidade, custo_unitario, carencia_dias))`)
...
export async function criarOperacao({ talhao_id, safra_id, categoria, data_operacao, custo_aplicacao, observacoes, receituario_agronomo, receituario_crea, receituario_emissao, insumos_usados }) {
  const catInfo = getCategoriaInfo(categoria)
  const { data: op, error: opErr } = await supabase.from('operacoes')
    .insert({ talhao_id, safra_id: safra_id||null, categoria, tipo: catInfo.label, data_operacao, custo_aplicacao: Number(custo_aplicacao)||0, observacoes: observacoes||null, receituario_agronomo: receituario_agronomo||null, receituario_crea: receituario_crea||null, receituario_emissao: receituario_emissao||null })
    .select().single()
  if (opErr) throw opErr
  if (insumos_usados && insumos_usados.length > 0) {
    const { error: insErr } = await supabase.from('operacao_insumos')
      .insert(insumos_usados.map(i => ({ operacao_id: op.id, insumo_id: i.insumo_id, dose: Number(i.dose), dose_unidade: i.dose_unidade, quantidade_total: Number(i.quantidade_total), custo_total: Number(i.custo_total) })))
...
export async function listarSafras(fazendaId) {
  const { data, error } = await supabase.from('safras').select('id, nome, ativa').eq('fazenda_id', fazendaId).order('created_at', { ascending: false })
  if (error) throw error
  return data || []
}
