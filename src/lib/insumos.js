...
export async function listarInsumos(fazendaId) {
  ...
  return (data || []).map(i => ({
    ...i,
    estoque: i.estoque?.[0] || { quantidade_atual: 0, quantidade_inicial: 0, quantidade_minima: 0, status: 'vazio' }
  }))
}
...
export async function criarInsumo({ fazenda_id, nome, classe, unidade, custo_unitario, carencia_dias, fornecedor }) {
  ...
  await supabase
    .from('estoque')
    .insert({ insumo_id: insumo.id, quantidade_atual: 0, quantidade_inicial: 0, quantidade_minima: 0 })
  return insumo
}
...
export async function atualizarEstoque(insumoId, { quantidade_atual, quantidade_inicial, quantidade_minima }) {
  const { data, error } = await supabase
    ...
    .update({
      ...
      quantidade_inicial: Number(quantidade_inicial) || 0,
