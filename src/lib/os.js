...
export const OPERACOES_MAE = [
  {
    id: 'aplicacao_tratorizada',
    label: 'Aplicação Tratorizada Terrestre',
    servicos: [
  ...
  {
    id: 'adubacao_autopropelido',
    label: 'Adubação Autopropelido',
    servicos: [
...
export function getOperacaoMae(id) {
  return OPERACOES_MAE.find(o => o.id === id)
}
...
export const BICOS_COMUNS = [
  'Leque TT 110-02',
  'Leque TT 110-03',
  'Leque TT 110-04',
  'Cônico JD 14-4',
  'Cônico JD 14-6',
  'Induzido ADGA 110-02',
  'Induzido ADGA 110-03',
  'Turbo TeeJet TJ 60-11002',
  'Turbo TeeJet TJ 60-11004',
  'Outro'
]
...
export async function criarOS({
  ...
  const { data: os, error: osErr } = await supabase
    ...
    .insert({
      fazenda_id,
      talhao_id: talhao_ids[0],
      numero,
      operacao_mae,
      servico,
      operacao_recomendada: servInfo?.label || servico,
      categoria: servInfo?.categoria || categoria,
      ...
      vazao_lha: vazao_lha ? Number(vazao_lha) : null,
      bico: bico || null,
...
export async function fecharOSCompleta({
  os_id,
  talhoes_ids,
  data_execucao,
  custo_aplicacao,
  ...
  for (const ins of insumos_reais || []) {
    await supabase
      .from('os_insumos')
      .update({
        dose_real: Number(ins.dose_real) || 0,
        quantidade_real: Number(ins.quantidade_real) || 0,
        custo_real: Number(ins.custo_real) || 0
      })
      .eq('id', ins.id)
  }
  const operacaoIds = []
  for (const talhao_id of talhoes_ids) {
    const { data, error } = await supabase.rpc('fechar_os_v2', {
      p_os_id: os_id,
      p_talhao_id: talhao_id,
      p_data_execucao: data_execucao,
      p_custo_aplicacao: Number(custo_aplicacao) || 0,
      p_observacoes: observacoes || null,
      p_receituario_agronomo: receituario_agronomo || null,
      p_receituario_crea: receituario_crea || null
    })
    if (error) throw error
  ...
  await supabase
    ...
    .update({
      status: 'concluida',
      data_execucao,
      custo_aplicacao_real: Number(custo_aplicacao) || 0,
