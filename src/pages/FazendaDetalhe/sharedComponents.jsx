...
export function TalhaoMiniKpi({ label, value }) {
  return (
    <div style={talhaoMiniKpiStyle}>
      ...
      <strong style={{ display: 'block', marginTop: 5, color: C.textDk, fontSize: 14, fontFamily: 'Georgia, serif' }}>
        {value}
      </strong>
    </div>
  )
...
export function MetricCard({ label, value, tone }) {
  return (
    <div style={metricCardStyle}>
      <p style={eyebrowStyle}>{label.toUpperCase()}</p>
      <strong style={{ display: 'block', marginTop: 6, color: tone, fontSize: 21, fontFamily: 'Georgia, serif' }}>
        {value}
      </strong>
    </div>
  )
...
export function CustoPizzaCard() {
  ...
  return (
    <div
      style={{ ...metricCardStyle, display: 'grid', gridTemplateColumns: '88px 1fr', gap: 12, alignItems: 'center' }}
    >
      ...
      <div>
        ...
        <strong
          style={{ display: 'block', marginTop: 5, color: C.greenDp, fontSize: 20, fontFamily: 'Georgia, serif' }}
        >
          R$ 0,00
        </strong>
        <div style={{ display: 'grid', gap: 3, marginTop: 8 }}>
          {fatias.map(fatia => (
            <div
              key={fatia.nome}
              style={{ display: 'flex', gap: 6, alignItems: 'center', fontSize: 10, color: C.textMid }}
            >
...
export function SmartInsightCard({ title, status, actionLabel, onAction, insights }) {
  ...
  return (
    <div style={smartCardStyle}>
      ...
      <button onClick={onAction} style={smartActionStyle}>
        {actionLabel}
      </button>
    </div>
  )
      ...
      <div style={{ display: 'grid', gap: 8, marginTop: 12 }}>
        {items.map(item => (
          <div key={item} style={{ display: 'flex', gap: 8, alignItems: 'flex-start' }}>
            <span
              style={{ width: 7, height: 7, borderRadius: 99, background: C.greenDp, marginTop: 5, flexShrink: 0 }}
            />
  ...
  return (
    <div
      style={{
        background: C.bg,
        borderRadius: 12,
        border: `1px solid ${open ? info.cor : C.border}`,
        overflow: 'hidden',
        marginBottom: 6,
      ...
      >
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <p style={{ margin: 0, fontSize: 10, color: C.textMid, fontFamily: 'monospace' }}>
            {op.data_operacao} · {op.insumos?.length || 0} insumo{(op.insumos?.length || 0) !== 1 ? 's' : ''}
          </p>
        </div>
        <p
          style={{ margin: 0, fontSize: 12, fontWeight: 700, color: info.cor, fontFamily: 'monospace', flexShrink: 0 }}
        >
          {money(totalOp)}
        </p>
        <span
      ...
      {open && (
        <div style={{ padding: '0 14px 12px', borderTop: `1px solid ${C.borderSoft}`, background: C.bgSoft }}>
          <div style={{ display: 'flex', gap: 8, marginTop: 10, marginBottom: 8 }}>
            {[
              { l: 'INSUMOS', v: totalInsumos, c: C.greenDp },
              { l: 'APLICACAO', v: Number(op.custo_aplicacao || 0), c: C.amberDk }
            ].map(x => (
              <div
                key={x.l}
                style={{
              ...
              >
                <p style={{ margin: 0, fontSize: 7, color: C.textDim, fontFamily: 'monospace', letterSpacing: '1px' }}>
                  {x.l}
                </p>
                <p style={{ margin: '2px 0 0', fontSize: 12, fontWeight: 700, color: x.c, fontFamily: 'monospace' }}>
                  {money(x.v)}
                </p>
              </div>
            ))}
                ...
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <p style={{ margin: 0, fontSize: 9, color: C.textMid, fontFamily: 'monospace' }}>
                      {i.dose} {i.dose_unidade} · {i.quantidade_total} {i.insumo?.unidade} total
                    </p>
                  </div>
                  <p style={{ margin: 0, fontSize: 11, fontWeight: 700, color: info.cor, fontFamily: 'monospace' }}>
                    {money(i.custo_total)}
                  </p>
                </div>
              ))}
          ...
          {op.receituario_agronomo && (
            ...
            >
              ...
              <p style={{ margin: '2px 0 0', fontSize: 11, color: C.textDk }}>
                {op.receituario_agronomo} - {op.receituario_crea}
              </p>
            </div>
          )}
          {op.observacoes && (
            ...
            >
              <p style={{ margin: 0, fontSize: 8, color: C.textDim, fontFamily: 'monospace', letterSpacing: '1px' }}>
                OBSERVACOES
              </p>
    ...
    >
      ...
      >
        ...
        <form onSubmit={onSubmit}>
          {[
            ['CODIGO *', 'codigo', 'T1', 'text'],
            ['AREA (ha) *', 'area_ha', '28.5', 'number']
          ].map(([l, k, ph, t]) => (
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <button type="button" onClick={onClose} style={{ ...secondaryActionStyle, flex: 1 }}>
              Cancelar
            </button>
            <button
              type="submit"
              disabled={salvando}
              style={{ ...primaryActionStyle, flex: 2, opacity: salvando ? 0.65 : 1 }}
            >
