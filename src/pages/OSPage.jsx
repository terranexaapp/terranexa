...
export function OSPage() {
  ...
  useEffect(() => {
    ...
    supabase
      .from('talhoes')
      .select('id, codigo, cultura, area_ha')
  ...
  async function carregar() {
    ...
    setErro(null)
    try {
      setOsList(await listarOS(fazendaId))
    } catch (e) {
      console.error(e)
      setErro(e)
    } finally {
    ...
    <div style={{ minHeight: '100vh', background: C.bgSoft }}>
      ...
      <main style={{ maxWidth: 900, margin: '0 auto', padding: '20px 16px 96px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8, marginBottom: 16 }}>
          {[
            { l: 'PENDENTES', v: counts.pendente, c: C.amber },
            { l: 'CONCLUÍDAS', v: counts.concluida, c: C.greenDp },
            { l: 'CANCELADAS', v: counts.cancelada, c: C.textDim }
          ].map(s => (
            <div
              key={s.l}
          ...
          {[
            ...
            <button
              ...
              style={{
                ...
                borderRadius: 99,
                cursor: 'pointer',
                background: filtro === f.id ? C.greenDp : C.bg,
                color: filtro === f.id ? C.bg : C.textMid,
                border: `1px solid ${filtro === f.id ? C.greenDp : C.border}`,
                fontSize: 10,
                fontFamily: 'monospace',
                fontWeight: 700
              }}
      ...
      {modal === 'equipes' && (
        <EquipesModal
          ...
          onSaved={() => listarEquipes(fazendaId).then(setEquipes)}
        />
      )}
    </div>
  )
      ...
      <div style={{ padding: '12px 14px', display: 'flex', alignItems: 'flex-start', gap: 12 }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap', marginBottom: 4 }}>
            <span style={{ fontSize: 12, fontWeight: 700, color: C.textDim, fontFamily: 'monospace' }}>
              {os.numero}
            </span>
            <span
              style={{
          ...
          <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
            {todosTalhoes.map(t => (
              ...
              >
                {t.codigo} · {Number(t.area_ha).toFixed(0)} ha
              </span>
            ))}
            {os.area_parcial_ha && (
              <span
              ...
              >
                Parcial: {Number(os.area_parcial_ha).toFixed(0)} ha ({Number(os.area_percentual || 0).toFixed(0)}%)
              </span>
            )}
            {os.prazo && (
              <span style={{ fontSize: 10, color: C.textMid, fontFamily: 'monospace' }}>Prazo: {os.prazo}</span>
            )}
          </div>
          {os.os_insumos?.length > 0 && (
            <p style={{ margin: '4px 0 0', fontSize: 10, color: C.textDim }}>
              {os.os_insumos.length} insumo{os.os_insumos.length > 1 ? 's' : ''} recomendado
              {os.os_insumos.length > 1 ? 's' : ''}
            </p>
          )}
        </div>
        ...
        <div style={{ padding: '0 14px 12px', borderTop: `1px solid ${C.borderSoft}`, background: C.bgSoft }}>
          ...
          {os.observacoes && (
            ...
            >
              ...
              >
                INSTRUÇÕES DO AGRÔNOMO
              </p>
              <p style={{ margin: '3px 0 0', fontSize: 11, color: C.textDk, lineHeight: 1.5 }}>{os.observacoes}</p>
            </div>
          )}
          {os.os_insumos?.length > 0 && (
            <>
              ...
              >
                {os.status === 'concluida' ? 'INSUMOS EXECUTADOS' : 'INSUMOS RECOMENDADOS'}
              </p>
              {os.os_insumos.map(i => (
                <div
                  key={i.id}
                ...
                >
                  <div style={{ flex: 1 }}>
                    ...
                    <p style={{ margin: 0, fontSize: 9, color: C.textMid, fontFamily: 'monospace' }}>
                      ...
                      {os.status === 'concluida' && i.dose_real > 0 && (
                        <span style={{ color: C.greenDp }}>
                          {' '}
                          · Real: {i.dose_real} {i.dose_unidade}
                        </span>
                      )}
                    </p>
                  </div>
                  {os.status === 'concluida' && i.custo_real > 0 && (
                    <p style={{ margin: 0, fontSize: 11, fontWeight: 700, color: C.greenDp, fontFamily: 'monospace' }}>
                      R$ {Number(i.custo_real).toFixed(2)}
                    </p>
                  )}
                </div>
              ))}
...
function Chip({ label, valor }) {
  return (
    <div style={{ background: C.bg, borderRadius: 8, padding: '5px 9px', border: `1px solid ${C.border}` }}>
      <p
        style={{
          margin: 0,
          fontSize: 7,
      ...
      <p style={{ margin: '2px 0 0', fontSize: 11, color: C.textDk, fontWeight: 600 }}>{valor}</p>
    </div>
  )
}
...
function NovaOSModal({ fazendaId, talhoes, insumos, equipes, prefill, onClose, onSaved }) {
  ...
  const areaTotal = talhoesSel.reduce((s, id) => {
    const t = talhoes.find(x => x.id === id)
    return s + Number(t?.area_ha || 0)
  }, 0)
  const areaEfetiva = areaParcial && Number(areaParcial) > 0 ? Math.min(Number(areaParcial), areaTotal) : areaTotal
  const percentualParcial =
    areaTotal > 0 && areaParcial && Number(areaParcial) > 0 ? Math.round((Number(areaParcial) / areaTotal) * 100) : null
  ...
  const filtrados = insumos
    .filter(
      i => (!busca || i.nome.toLowerCase().includes(busca.toLowerCase())) && !insumosRec.find(r => r.insumo_id === i.id)
    )
    .slice(0, 8)
  const precisaRec =
    servicoInfo &&
  ...
  async function handleSalvar() {
    ...
    if (!servico) {
      setErro('Selecione a natureza agrícola')
      return
    }
    ...
    try {
      await criarOS({
        ...
        talhao_ids: talhoesSel,
        operacao_mae: operacaoMae,
        servico,
        categoria: servicoInfo?.categoria || 'outros',
        ...
        cultura_alvo: culturaDaOrdem || null,
        vazao_lha: form.vazao_lha || null,
      ...
      setErro(err.message || 'Erro ao criar ordem de serviço')
      setSalvando(false)
    }
  }
    ...
    >
      ...
      >
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '16px 18px' }}>
            ...
            {OPERACOES_MAE.map((mae, idx) => (
              <button
                ...
                style={{
                  ...
                  padding: '8px 10px',
                  borderRadius: 9,
                  border: `1.5px solid ${operacaoMae === mae.id ? C.greenDp : C.border}`,
                  background: operacaoMae === mae.id ? C.greenLight : C.bg,
                  cursor: 'pointer',
                  textAlign: 'left'
                }}
              >
                <span
                  style={{
                    ...
                    height: 24,
                    borderRadius: 7,
                    background: operacaoMae === mae.id ? C.greenDp : C.bgSoft,
                    color: operacaoMae === mae.id ? C.bg : C.textDim,
                    display: 'flex',
                    alignItems: 'center',
                    justifyContent: 'center',
                    fontSize: 10,
                ...
                {maeInfo.servicos.map(s => (
                  <button
                    ...
                    style={{
                      ...
                      padding: '9px 12px',
                      borderRadius: 9,
                      border: `1.5px solid ${servico === s.id ? C.greenDp : C.border}`,
                      background: servico === s.id ? C.greenLight : C.bg,
                      cursor: 'pointer',
                      textAlign: 'left'
                    }}
                  >
                    <div
                      style={{
                        ...
                        height: 8,
                        borderRadius: 2,
                        background: servico === s.id ? C.greenDp : C.border,
                        flexShrink: 0
                      }}
                    />
          ...
          <SecTitle
            label={`3. TALHÕES (${talhoesSel.length} selecionado${talhoesSel.length !== 1 ? 's' : ''} · ${areaTotal.toFixed(1)} ha)`}
          />
          <div style={{ marginBottom: 10 }}>
            <input
              value={buscaTalhao}
              onChange={e => setBuscaTalhao(e.target.value)}
              placeholder="Digite para buscar talhão..."
            ...
            {buscaTalhao && (
              ...
              >
                {talhoesFiltrados.length === 0 ? (
                  <p style={{ margin: 0, padding: '8px 10px', fontSize: 11, color: C.textDim }}>
            ...
            {talhoesSelecionados.length === 0 && (
              <p style={{ margin: '4px 0 0', fontSize: 10, color: C.textDim }}>
                Busque e selecione um talhão para preencher a cultura automaticamente.
              </p>
            )}
          </div>
              ...
              <F label="ÁREA PARCIAL (ha) — opcional">
                <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                  <input
                    ...
                    max={areaTotal}
                    value={areaParcial}
                    onChange={e => setAreaParcial(e.target.value)}
                    placeholder={'Max ' + areaTotal.toFixed(0) + ' ha'}
                    style={{ ...inp, flex: 2 }}
                  />
                  {percentualParcial !== null && (
                    <div
                      style={{
                        flex: 1,
                        padding: '10px',
                    ...
                    >
                      ...
                      <p style={{ margin: 0, fontSize: 8, color: C.amberDk, fontFamily: 'monospace' }}>DA ÁREA</p>
                    </div>
                  )}
                </div>
              </F>
              {areaParcial && Number(areaParcial) > 0 && (
                <p style={{ margin: '4px 0 0', fontSize: 10, color: C.amberDk }}>
                  Recomendação calculada sobre <strong>{Number(areaParcial).toFixed(1)} ha</strong>
                </p>
              )}
            </div>
          )}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            ...
            <F label="VAZÃO (L/ha)">
              <input
                ...
                step="0.1"
                min="0"
                value={form.vazao_lha}
                onChange={e => set('vazao_lha', e.target.value)}
                placeholder="Ex: 80"
                style={inp}
              />
            </F>
          ...
          {usaBico && (
            <F label="BICO UTILIZADO">
              <div style={{ position: 'relative' }}>
                <input
                  value={form.bico}
                  onChange={e => set('bico', e.target.value)}
                  onFocus={() => setBicosDropdown(true)}
                  onBlur={() => setTimeout(() => setBicosDropdown(false), 200)}
                ...
                {bicosDropdown && (
                  ...
                  >
                    {BICOS_COMUNS.filter(b => !form.bico || b.toLowerCase().includes(form.bico.toLowerCase())).map(
                      b => (
                        <div
                          key={b}
                          onMouseDown={() => set('bico', b)}
                          style={{
                            padding: '9px 12px',
                            cursor: 'pointer',
                            fontSize: 12,
          ...
          <F label="OBSERVAÇÕES / INSTRUÇÕES PARA O EXECUTOR">
            <textarea
              ...
              placeholder="Instruções de aplicação, condições ideais, cuidados especiais..."
              rows={3}
          ...
          {insumosRec.length === 0 ? (
            ...
            >
              ...
              <p style={{ margin: '3px 0 0', fontSize: 10, color: C.textVery }}>
                A ordem de serviço pode ser criada sem insumos
              </p>
            </div>
          ) : (
                ...
                >
                  ...
                  <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                    <input
                      ...
                      step="0.001"
                      min="0"
                      value={i.dose_recomendada}
                      onChange={e =>
                        setInsumosRec(p =>
                          p.map((x, n) => (n === idx ? { ...x, dose_recomendada: e.target.value } : x))
                        )
                      }
                      placeholder="Dose"
                      style={{
                    ...
                    <input
                      ...
                      onChange={e =>
                        setInsumosRec(p => p.map((x, n) => (n === idx ? { ...x, dose_unidade: e.target.value } : x)))
                      }
                      style={{
                        flex: 1,
                        padding: '8px 4px',
                    ...
                    />
                    {areaEfetiva > 0 && i.dose_recomendada > 0 && (
                      <div
                        style={{
                          flex: 1,
                          padding: '8px',
                      ...
                      >
                        ...
                        >
                          R$ {calcularCustoInsumo(i.custo_unitario, i.dose_recomendada, areaEfetiva).toFixed(0)}
                        </p>
                        <p style={{ margin: 0, fontSize: 8, color: C.textDim, fontFamily: 'monospace' }}>
                          {(Number(i.dose_recomendada) * areaEfetiva).toFixed(1)} {i.unidade}
                        </p>
                      </div>
                    )}
                  </div>
                  {i.carencia_dias > 0 && (
                    <p style={{ margin: '4px 0 0', fontSize: 9, color: C.amberDk, fontFamily: 'monospace' }}>
                      Carência: {i.carencia_dias} dias
                    </p>
                  )}
                </div>
              ))}
...
function ExecutarOSModal({ os, onClose, onSaved }) {
  ...
  const areaRef = os.area_parcial_ha
    ? Number(os.area_parcial_ha)
    : todosTalhoes.reduce((s, t) => s + Number(t?.area_ha || 0), 0)
  ...
  const [insumosReais, setInsumosReais] = useState(
    (os.os_insumos || []).map(i => ({
      ...
      custo_unitario: i.insumo?.custo_unitario,
      carencia_dias: i.insumo?.carencia_dias,
      dose_recomendada: i.dose_recomendada,
      dose_unidade: i.dose_unidade,
      dose_real: i.dose_recomendada,
      quantidade_real: Math.round(Number(i.dose_recomendada) * areaRef * 1000) / 1000,
      custo_real: Math.round(calcularCustoInsumo(i.insumo?.custo_unitario, i.dose_recomendada, areaRef) * 100) / 100
    }))
  )
  const [salvando, setSalvando] = useState(false)
  const [erro, setErro] = useState('')
  ...
  function atualizarReal(idx, patch) {
    setInsumosReais(prev =>
      prev.map((i, n) => {
        if (n !== idx) return i
        const u = { ...i, ...patch }
        if ('dose_real' in patch) {
          u.quantidade_real = Math.round(Number(u.dose_real) * areaRef * 1000) / 1000
          u.custo_real = Math.round(calcularCustoInsumo(u.custo_unitario, u.dose_real, areaRef) * 100) / 100
        }
        return u
      })
    )
  ...
  async function handleFechar() {
    ...
    setSalvando(true)
    try {
      await fecharOSCompleta({
        os_id: os.id,
        talhoes_ids: todosTalhoes.map(t => t.id),
        data_execucao: dataExecucao,
        custo_aplicacao: Number(custoAplicacao) || 0,
        observacoes,
        receituario_agronomo: recAgronomo,
        receituario_crea: recCrea,
    ...
    >
      ...
      >
        ...
        <div style={{ padding: '16px 18px' }}>
          {(os.cultura_alvo || os.vazao_lha || os.bico || os.equipe) && (
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 12 }}>
              {os.cultura_alvo && <Chip label="CULTURA" valor={os.cultura_alvo} />}
              {os.vazao_lha && <Chip label="VAZÃO" valor={os.vazao_lha + ' L/ha'} />}
              {os.bico && <Chip label="BICO" valor={os.bico} />}
              {os.equipe && <Chip label="EQUIPE" valor={os.equipe.nome} />}
            </div>
          )}
          {os.observacoes && (
            <div
            ...
            >
              ...
              <p style={{ margin: 0, fontSize: 12, color: C.textDk, lineHeight: 1.6 }}>{os.observacoes}</p>
            </div>
          )}
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 12 }}>
            <F label="DATA DE EXECUÇÃO *">
              <input
                type="date"
                required
                value={dataExecucao}
            ...
            <F label="CUSTO DE APLICAÇÃO (R$)">
              <input
                type="number"
                step="0.01"
                min="0"
                value={custoAplicacao}
                onChange={e => setCustoAplicacao(e.target.value)}
                placeholder="0.00"
                style={inp}
              />
            </F>
            ...
            >
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 8 }}>
                <F label="AGRÔNOMO">
                  <input
                    value={recAgronomo}
                    onChange={e => setRecAgronomo(e.target.value)}
                    placeholder="Eng. Agrônomo..."
                    style={inp}
                  />
                </F>
                <F label="CREA">
                  <input
                    value={recCrea}
                    onChange={e => setRecCrea(e.target.value)}
                    placeholder="CREA-PE 12345"
                    style={inp}
                  />
                </F>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                      <div>
                        ...
                        <div style={{ display: 'flex', gap: 4 }}>
                          <input
                            ...
                            min="0"
                            value={i.dose_real}
                            onChange={e => atualizarReal(idx, { dose_real: e.target.value })}
                            style={{
                              flex: 2,
                              padding: '8px',
                              background: C.bg,
                      ...
                      <div>
                        ...
                        >
                          ...
                          <p style={{ margin: 0, fontSize: 9, color: C.textDim, fontFamily: 'monospace' }}>
                            {Number(i.quantidade_real).toFixed(2)} {i.unidade}
                          </p>
                        </div>
                      </div>
                    </div>
          ...
          <F label="OBSERVAÇÕES DA EXECUÇÃO">
            <textarea
              value={observacoes}
          ...
          >
            ...
            {[
              ...
              { l: 'Aplicação', v: Number(custoAplicacao || 0) }
            ].map(r => (
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
              ...
              <div style={{ textAlign: 'right' }}>
                <p style={{ margin: 0, fontSize: 16, fontWeight: 700, color: C.greenDp, fontFamily: 'Georgia, serif' }}>
                  R$ {custoTotal.toFixed(2)}
                </p>
                <p style={{ margin: 0, fontSize: 10, color: C.textMid, fontFamily: 'monospace' }}>
                  R$ {custoHa.toFixed(2)}/ha · {areaRef.toFixed(1)} ha
                </p>
              </div>
            </div>
          </div>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            >
              {salvando ? 'FECHANDO...' : 'FECHAR ORDEM + REGISTRAR'}
            </button>
          </div>
        </div>
      </div>
...
function EquipesModal({ fazendaId, equipes, onClose, onSaved }) {
  ...
  async function handleCriar(e) {
    ...
    setSalvando(true)
    try {
      await criarEquipe({ fazenda_id: fazendaId, nome, responsavel })
      setNome('')
      setResponsavel('')
      onSaved()
    } catch (err) {
    ...
    >
      ...
      >
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '16px 18px' }}>
            ...
            <div style={{ marginBottom: 20 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                {equipes.map(eq => (
                  ...
                  >
                    <div style={{ flex: 1 }}>
                      <p style={{ margin: 0, fontSize: 13, fontWeight: 700, color: C.textDk }}>{eq.nome}</p>
                      <p style={{ margin: 0, fontSize: 11, color: C.textMid }}>Resp: {eq.responsavel}</p>
                    </div>
                    <button
          ...
          <p
            style={{ margin: '0 0 10px', fontSize: 9, color: C.textDim, fontFamily: 'monospace', letterSpacing: '2px' }}
          >
            NOVA EQUIPE
          </p>
          <form onSubmit={handleCriar}>
            <F label="NOME DA EQUIPE *">
              <input
                required
                value={nome}
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              ...
              >
                {salvando ? 'SALVANDO...' : 'CADASTRAR EQUIPE'}
              </button>
            </div>
          </form>
        </div>
...
function SecTitle({ label }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
      ...
      <div style={{ flex: 1, height: 1, background: C.greenDp + '33' }} />
    </div>
  )
}
