...
export async function criarTalhao({ fazenda_id, codigo, nome, cultura, area_ha, fase, geometria }) {
  const { data, error } = await supabase.from('talhoes')
    .insert({ fazenda_id, codigo, nome, cultura, area_ha, fase: fase || 'preparo', geometria })
    .select().single()
  if (error) throw error
  return data
}
...
export async function criarTalhoesEmLote(fazenda_id, talhoes) {
  const { data, error } = await supabase.from('talhoes')
    .insert(talhoes.map(t => ({ fazenda_id, codigo: t.codigo, nome: t.nome, cultura: t.cultura, area_ha: t.area_ha, fase: t.fase || 'preparo', geometria: t.geometria })))
    .select()
  if (error) throw error
  return data
}
...
export async function proximoCodigoTalhao(fazendaId) {
  const { data } = await supabase.from('talhoes').select('codigo').eq('fazenda_id', fazendaId)
  const nums = (data || []).map(t => parseInt(t.codigo.replace(/\D/g, ''))).filter(n => !isNaN(n))
  return `T${nums.length === 0 ? 1 : Math.max(...nums) + 1}`
}
