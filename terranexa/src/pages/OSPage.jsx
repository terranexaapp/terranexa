...
import { listarOS, criarOS, fecharOSCompleta, cancelarOS, OPERACOES_MAE, getOperacaoMae, STATUS_OS, PRIORIDADE_OS, CULTURAS_ALVO, BICOS_COMUNS } from '../lib/os'
...
function NovaOSModal({fazendaId, talhoes, insumos, equipes, onClose, onSaved}) {
  ...
  const [form, setForm] = useState({cultura_alvo:'',vazao_lha:'',bico:'',equipe_id:'',prioridade:'media',prazo:'',receituario_agronomo:'',receituario_crea:'',observacoes:''})
    ...
    <div onClick={onClose} style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.55)',zIndex:1000,display:'flex',alignItems:'center',justifyContent:'center',padding:12,overflowY:'auto'}}>
      <div onClick={e=>e.stopPropagation()} style={{background:C.bg,borderRadius:18,width:'100%',maxWidth:560,maxHeight:'94vh',display:'flex',flexDirection:'column'}}>
        <div style={{padding:'16px 18px',borderBottom:`1px solid ${C.border}`,display:'flex',justifyContent:'space-between',alignItems:'center',flexShrink:0}}>
          <div><p style={{margin:0,fontSize:9,color:C.textDim,fontFamily:'monospace',letterSpacing:'2px'}}>ABRIR ORDEM DE SERVIÇO</p><h2 style={{margin:'4px 0 0',fontSize:17,color:C.textDk,fontWeight:700,fontFamily:'Georgia, serif'}}>Nova OS</h2></div>
        ...
        <div style={{flex:1,overflowY:'auto',padding:'16px 18px'}}>
          ...
          <SecTitle label="1. OPERAÇÃO MÃE"/>
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:6,marginBottom:14}}>
            {OPERACOES_MAE.map(mae=>(
              <button key={mae.id} onClick={()=>{setOperacaoMae(mae.id);setServico('')}} style={{padding:'10px 12px',borderRadius:10,border:`1.5px solid ${operacaoMae===mae.id?C.greenDp:C.border}`,background:operacaoMae===mae.id?C.greenLight:C.bg,cursor:'pointer',textAlign:'left'}}>
                <p style={{margin:0,fontSize:12,fontWeight:700,color:C.textDk}}>{mae.label}</p>
                <p style={{margin:'2px 0 0',fontSize:9,color:C.textDim}}>{mae.servicos.length} serviços</p>
              </button>
            ))}
          </div>

          {maeInfo&&<>
            <SecTitle label="2. SERVIÇO"/>
          ...
          <div style={{display:'flex',flexDirection:'column',gap:4,marginBottom:10}}>
            {talhoes.map(t=>(
              <div key={t.id} onClick={()=>toggleTalhao(t.id)} style={{display:'flex',alignItems:'center',gap:10,padding:'9px 12px',borderRadius:9,border:`1.5px solid ${talhoesSel.includes(t.id)?C.greenDp:C.border}`,background:talhoesSel.includes(t.id)?C.greenLight:C.bg,cursor:'pointer'}}>
                <div style={{width:18,height:18,borderRadius:4,border:`2px solid ${talhoesSel.includes(t.id)?C.greenDp:C.border}`,background:talhoesSel.includes(t.id)?C.greenDp:'transparent',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}>
                  {talhoesSel.includes(t.id)&&<span style={{color:C.bg,fontSize:11}}>✓</span>}
                </div>
                <p style={{margin:0,fontSize:12,fontWeight:700,color:C.textDk}}>{t.codigo}</p>
                <p style={{margin:0,fontSize:10,color:C.textMid}}>{t.cultura?.charAt(0).toUpperCase()+t.cultura?.slice(1)}</p>
                <p style={{margin:0,fontSize:10,color:C.textMid,marginLeft:'auto',fontFamily:'monospace'}}>{Number(t.area_ha).toFixed(1)} ha</p>
              </div>
            ))}
          </div>
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}>
            <F label="CULTURA ALVO">
              <select value={form.cultura_alvo} onChange={e=>set('cultura_alvo',e.target.value)} style={inp}>
                <option value="">Selecione...</option>
                {CULTURAS_ALVO.map(c=><option key={c} value={c}>{c}</option>)}
              </select>
            </F>
            <F label="VAZÃO (L/ha)">
          ...
          {insumosRec.length===0?(
            <div style={{padding:'16px',textAlign:'center',background:C.bgSoft,borderRadius:12,border:`1px dashed ${C.border}`,marginBottom:12}}>
              ...
              <p style={{margin:'3px 0 0',fontSize:10,color:C.textVery}}>OS pode ser criada sem insumos</p>
            </div>
          ):(
          ...
          <div style={{display:'flex',gap:8}}>
            ...
            <button onClick={handleSalvar} disabled={salvando} style={{flex:2,padding:'13px',background:salvando?C.textDim:C.greenDp,color:C.bg,border:'none',borderRadius:10,fontSize:12,fontFamily:'monospace',fontWeight:700,letterSpacing:'2px',cursor:salvando?'not-allowed':'pointer'}}>{salvando?'CRIANDO...':'CRIAR OS'}</button>
          </div>
        </div>
      </div>
    </div>
      ...
      <div onClick={e=>e.stopPropagation()} style={{background:C.bg,borderRadius:18,width:'100%',maxWidth:520,maxHeight:'92vh',overflowY:'auto'}}>
        ...
        <div style={{padding:'16px 18px'}}>
          ...
          <div style={{display:'flex',gap:8}}>
            ...
            <button onClick={handleFechar} disabled={salvando} style={{flex:2,padding:'12px',background:salvando?C.textDim:C.greenDp,color:C.bg,border:'none',borderRadius:10,fontSize:11,fontFamily:'monospace',fontWeight:700,cursor:salvando?'not-allowed':'pointer'}}>{salvando?'FECHANDO...':'FECHAR OS + REGISTRAR'}</button>
          </div>
        </div>
      </div>
    </div>
